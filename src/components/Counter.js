...
import React, { isValidElement, useState } from 'react'; /* STEP 0 */

export default function Counter() {
  /* STEP 1 */ //'count', 'setCount'
  const [count, setCount] = useState(0);
  let countIsEven = 0 === count % 2;
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: (countIsEven ? 'royalblue' : 'crimson') /* STEP 2 */
  };
  ...
  return (
    <div className='widget-counter container'>
      ...
      <div id='count' style={style}>
        Number {count} is{countIsEven ? 'even' : 'odd'/* STEP 3*/}
      </div>
      <div>
